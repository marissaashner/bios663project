...
```{r}
## AIC Comparison
aic <- c(AIC(model1), AIC(model2), AIC(modelback))
bic <- c(BIC(model1), BIC(model2), BIC(modelback))
adjR <- c( summary(model1)$adj.r.sq,  summary(model2)$adj.r.sq,  summary(modelback)$adj.r.sq)

selection <- data.frame(model = c("Model1: Full Model", "Model2: No Intercept", "Model3: Stepwise Model"), AICs = aic, BICs = bic, Adjusted.R.Squared = adjR)
