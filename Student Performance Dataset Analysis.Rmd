...
ggplot(student_abs_23, aes(absences, G3)) +
  geom_jitter() +
  geom_smooth()
...
ggplot(student_abs_bin, aes(absences, G3)) +
  geom_jitter() +
  geom_smooth()
...
** HOW TO PICK WHAT TO REMOVE ** should we just take out the intercept? (tried in model2, looks good! )
...
$$y = \beta_0 + \beta_1X_1 + ... + \beta_{39}X_{39} + \epsilon$$
...
Since, from the collinearity analysis, several variables were significantly colinear with the intercept, we next build a model identical to the full model with the removal of the intercept. 
...
** maybe only try these on model2 since it performed better originally ** 
...
```{r}
modelback2 <- step(model2, direction = "backward", trace = F)
modelback2 

modelboth2 <- step(model2, direction = "both", trace = F)
modelboth2 

modelforward2 <- step(model2, direction = "forward", trace = F)
modelforward2
```
...
Backward Search: 
Variables to keep:
School, sex, age, address, Fedu, Mjob, Fjob, traveltime, studytime, failures, schoolsup, famsup, paid, higher, internet, freetime, goout, health, absences
...
DOES NOT give same exact answer if we use model1 with intercept or model2 without to do the stepwise process. Intercept is not included in the second set of models the stepwise process gives us. 

So in summary, modelback = modelboth = modelback2 = modelboth2 and modelforward = model2 (all variables with no intercept). ***UPDATE*** I think modelback from model1 and model2 are NOT the same... model2 is what gives the best without the intercept.
...
```{r}
stud_resid = rstudent(modelback2)
dat_resid <- data.frame(RESID = stud_resid, ABS = abs(stud_resid), PRED = modelback2$fitted.values)
dat_resid %>% arrange(-ABS) %>% filter(ABS > 2)
```
...
```{r}
p <- modelback2$coefficients %>% length()
n <- nrow(student_d)
levcutoff <- 2*p/n
lev <- hatvalues(modelback2)
cook <- cooks.distance(modelback2)
```
...
```{r}
## AIC Comparison
aic <- c(AIC(model1), AIC(model2), AIC(modelback2))
bic <- c(BIC(model1), BIC(model2), BIC(modelback2))
adjR <- c( summary(model1)$adj.r.sq,  summary(model2)$adj.r.sq,  summary(modelback2)$adj.r.sq)

selection <- data.frame(model = c("Model1", "Model2", "Model3"), AICs = aic, BICs = bic, Adjusted.R.Squared = adjR)
