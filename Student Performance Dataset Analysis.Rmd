...
We looked at the distribution of the outcome variable, final grade, and noticed that the distribution was fairly normal except for a number of 0's (see Figure 2 in the Appendix). Looking further, all of these students had positive grades for the first and second periods, so it didn't seem to make practical sense that their final grades were zero. Since we were not given more information on what these zeros meant, whether it was for students moving, dropping out, or are missing values, we decided to drop the values from the data for our purposes.
...
The covariates with the highest VIF values are shown below.
...
Adjusted $R^2$ value for this model is `r summary(model.int)$adj.r.sq`. Interestingly, it appears that the effects of studytime are reduced for high values of goout; this may suggest that going out increases fatigue or distractedness, leading to a decreased benefit from studying. 
