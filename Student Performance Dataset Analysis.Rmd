...
We looked at the distribution of the outcome variable, final grade, and noticed that the distribution was fairly normal except for a number of 0's. Looking further, all of these students had positive grades for the first and second periods, so it didn't seem to make practical sense that their final grades were zero. Since we were not given more information on what these zeros meant, whether it was for students moving, dropping out, or are missing values, we decided to drop the values from the data for our purposes.
...
We additionally had to perform some scaling of predictors in order to avoid problems of collinearity with these variables and the intercept. The first of these variables was the absences variable. While most of our variables were factors or numeric variables with small ranges, the absence variable ranged from 0 to 93 and was very skewed right. We decided to bin the absence variable into a 5 level numeric variable where 1 is absences less than 5, 2 is absences between 5 and 10, and so on, until 5 is replaced for absences greater than 20. The distribution before and after this variable transformation is shown in the plots below.
...
The second variable we transformed was the age variable. We subtracted each age from the minimum age so the range was change from 15-22 to 0-7.
...
Based on some exploratory data an analysis plots, which can be seen in the Appendix, we decided to remove the variable "guardian" from our dataset since it didn't seem to have any influence on the outcome. 
...
Since, from the collinearity analysis, several variables were significantly colinear with the intercept, we next try to build a model identical to the full model with the removal of the intercept to see how that affects the results of the model. It can be written as follows: 
...
It should be noted that the adjusted $R^2$ value for this model is `r summary(modelback)$adj.r.sq`. 
...
It should be noted that the adjusted $R^2$ value for this model is `r summary(model.int)$adj.r.sq`. 
...
It should be noted that the adjusted $R^2$ value for this model is `r summary(modelsmall)$adj.r.sq`. 
...
It should be noted that the adjusted $R^2$ value for this model is `r summary(smallint)$adj.r.sq`. 
...
It should be noted that the adjusted $R^2$ value for this model is `r summary(model6_all)$adj.r.sq`. 

## Model Selection and Interpretation

Aside from looking at the Adjusted $R^2$ to judge performance of a model, we can look at the AIC or SBC/BIC as well. We compare all of the models we built below, using all three of the performance measures listed above, as well as the number of parameters in the model. 
...
**Figure 5:** Residual analysis for each model.
