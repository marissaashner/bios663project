...
## Model 5: Using RegSubsets

```{r}
model5 <- lm(G3 ~ Mjobhealth + Mjobservices + Fjobteacher + failures + schoolsupyes + goout + health + absences, data = student_d)
summary(model5)
```
...
dat_resid <- data.frame(RESID = stud_resid, ABS = abs(stud_resid), PRED = model5$fitted.values)
dat_resid %>% arrange(-ABS) %>% filter(ABS > 2)
```
...
```{r}
## AIC Comparison
aic <- c(AIC(model1), AIC(model2), AIC(modelback), AIC(modelsmall), AIC(model5))
bic <- c(BIC(model1), BIC(model2), BIC(modelback), BIC(modelsmall), BIC(model5))
adjR <- c( summary(model1)$adj.r.sq,  summary(model2)$adj.r.sq,  summary(modelback)$adj.r.sq, summary(modelsmall)$adj.r.sq, summary(model5)$adj.r.sq)

selection <- data.frame(model = c("Model 1: Full Model", "Model 2: No Intercept", "Model 3: Stepwise Model", "Model 4: Reduced Stepwise Model", "Model 5: Best Subset Model"), AICs = aic, BICs = bic, Adjusted.R.Squared = adjR)
