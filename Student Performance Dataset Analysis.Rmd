...
student %>% 
  ggplot(aes(G3)) + geom_histogram(bins = 20)
```
#BB: is it odd that when we subset on G3 ne 0 we appear to lose more than just the counts at G3=0? We have ~80 people in the first histogram with a score of about 10 and only ~55 here. And we have no counts for 3 scores in the range above, but did not have these gaps initially. I'll look into this further
